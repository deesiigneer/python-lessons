while True:
    try:
        first_list = list(map(int, input('Введите перечень цифр через запятую: ').split(',')))
        first_list = [i for i in first_list if i % 2 != 0]
        second_list = list(map(int, input('Введите второй перечень цифр через запятую: ').split(',')))
